...
def observe(evidence: object, observation: object, *,
    ...
    if not isinstance(observation, CIObservation):
    ...
    if (type(observation.jobs) is not tuple or len(observation.jobs) != 2 or
            not all(isinstance(job, CIJobObservation) for job in observation.jobs)):
...
__all__ = [
    "APPROVED_REPOSITORY", "APPROVED_WORKFLOW", "CIJobObservation",
    "CIObservation", "OBSERVATION_VERSION", "observe",
]
