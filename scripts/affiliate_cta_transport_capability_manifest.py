...
def evaluate_connection_preflight(
    *, manifest: Any, setting_names: Any, clock_contract: Any, kill_runbook: Any,
    ...
    if OFFICIAL_WIRE_CONTRACT_VERSION is None or manifest.official_wire_contract_version != OFFICIAL_WIRE_CONTRACT_VERSION:
...
def _settings_schema_valid(value: Any) -> bool:
    if type(value) not in (tuple, frozenset):
        return False
    return len(value) == len(REQUIRED_SETTING_NAMES) and all(type(name) is str for name in value) and frozenset(value) == REQUIRED_SETTING_NAMES
...
def _manifest_valid(value: Any) -> bool:
    ...
    return (
        ...
        and type(value.official_wire_contract_version) is str
        and value.official_wire_contract_version == UNCONFIRMED
        and all((
            value.bounded_timeout, value.cancel_capability, value.no_retry,
            value.sequential, value.redaction, value.kill_revoke_checkpoints,
            value.late_result_discard,
        ))
        and value.max_items == MAX_SEQUENTIAL_ITEMS
    )


def _clock_contract_valid(value: Any) -> bool:
    return type(value) is ClockAdapterContract and (
        value.monotonic and value.unit == CLOCK_UNIT and value.finite_range
        and value.nondecreasing and value.process_local_limitation_acknowledged
    )


def _kill_runbook_valid(value: Any) -> bool:
    return type(value) is KillOperationsRunbookContract and (
        value.default_disabled and value.revoke_procedure and value.rollback_procedure
        and value.audit_reason and value.reenable_requires_new_approval
        and value.automatic_unlock is False
    )
