"""Durably fail one confirmed RUNNING generation without execution."""
...
class DurableJobFailedSafeResult:
...
def _result(status: str, reasons: tuple[str, ...], *, job_id: str | None = None,
            durable: bool = False, attempt_count: int | None = None,
            revision: int | None = None) -> DurableJobFailedSafeResult:
    return DurableJobFailedSafeResult(
        COORDINATOR_VERSION, status, durable, job_id, attempt_count, revision, reasons)


def _save_failure(saved: persistence.QueueSaveResult) -> DurableJobFailedSafeResult:
    if saved.status == "STALE_REVISION":
        return _result("FAILED_SAFE_CONFLICT", ("STALE_REVISION",))
    if saved.status == "RECOVERY_BLOCKED" and any(
            code in {"QUEUE_READ_BACK_FAILED", "QUEUE_SAVE_FAILED"}
            for code in saved.reason_codes):
        return _result("JOB_FAILED_SAFE_UNCERTAIN", ("RECOVERY_BLOCKED",))
    return _result("RECOVERY_BLOCKED", tuple(saved.reason_codes))


def fail_running_job_durably(
    store: Any, *, expected_job_id: Any, expected_attempt_count: Any,
) -> DurableJobFailedSafeResult:
    """Persist an explicit RUNNING -> FAILED_SAFE transition through one CAS."""
    ...
    if type(expected_attempt_count) is not int or expected_attempt_count < 1:
        return _result("FAILED_SAFE_REJECTED", ("EXECUTION_GENERATION_INVALID",))
    try:
        ...
        if len(matches) != 1:
            return _result("FAILED_SAFE_REJECTED", ("JOB_IDENTITY_NOT_CURRENT",))
        original = matches[0]
        if original.attempt_count != expected_attempt_count:
            return _result("FAILED_SAFE_REJECTED", ("EXECUTION_GENERATION_MISMATCH",))
        ...
        if (candidate is None or not validation.valid
                or validation.transition_class != "FAILED_SAFE_TRANSITION"
                or not core.validate_failed_safe_transition(
                    original, candidate, expected_job_id=expected_job_id)):
            reason = (transition.reason_code
                      if candidate is None else "FAILED_SAFE_TRANSITION_INVALID")
            return _result("FAILED_SAFE_REJECTED", (reason,))
        ...
        return _result("JOB_FAILED_SAFE_DURABLY", ("JOB_FAILED_SAFE_DURABLE",),
...
__all__ = [
    "COORDINATOR_VERSION", "DurableJobFailedSafeResult",
    "fail_running_job_durably",
]
