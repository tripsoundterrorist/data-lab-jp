...
def render(records: Any, *, selection_digest: Any, as_of: Any) -> CandidateResult:
    ...
    public_ids = tuple(record.get("public_id") if type(record) is dict else None for record in records)
    try:
        if not exact_selection.verify(public_ids, selection_digest):
            raise ValueError("SELECTION_DIGEST_MISMATCH")
    except ValueError as error:
        raise ValueError("CANARY_SELECTION_INVALID") from error
    cards=[]
    for record in records:
        if (type(record) is not dict or set(record) != {"public_id", "title", "observed_at", "fresh", "revalidation_status"}
                or type(record["public_id"]) is not str or exact_selection.PUBLIC_ID.fullmatch(record["public_id"]) is None
