...
class RevenueMvpReleaseGateTests(unittest.TestCase):
    ...
    def test_current_state_reports_exact_release_blockers(self):
        ...
        self.assertEqual(result.affiliate_deployment_status, "READY_FOR_DEPLOYMENT_REVIEW")
        self.assertTrue(result.affiliate_deployment_candidate)
        ...
        self.assertIn("IMPLEMENT_DMM_LIFECYCLE_CONDITIONS", result.next_actions)
        self.assertIn("IMPLEMENT_DMM_SORT_SEMANTICS_CONDITIONS", result.next_actions)
        self.assertNotIn("WAIT_FOR_DMM_LIFECYCLE_RESPONSE", result.next_actions)
        self.assertNotIn("WAIT_FOR_DMM_SORT_SEMANTICS_RESPONSE", result.next_actions)
