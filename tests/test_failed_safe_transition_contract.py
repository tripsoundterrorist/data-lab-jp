...
class FailedSafeContractTests(unittest.TestCase):
    def running(self):
        return job(state=queue.RUNNING)

    def complete(self, value=None):
        return queue.fail_job_safe(self.running() if value is None else value, expected_job_id="job-a")

    def test_version(self):
        self.assertEqual(queue.FAILED_SAFE_CONTRACT_VERSION, "0.1")

    def test_success(self):
        updated, result = self.complete()
        self.assertEqual(updated.state, queue.FAILED_SAFE)
        self.assertEqual(result.transition_status, "APPLIED")
        self.assertEqual((result.previous_state, result.new_state), (queue.RUNNING, queue.FAILED_SAFE))
    ...
    def test_validator_accepts(self):
        before = self.running()
        updated, _ = self.complete(before)
        self.assertTrue(queue.validate_failed_safe_transition(before, updated, expected_job_id="job-a"))

    def test_validator_rejects_modified_metadata(self):
        before = self.running()
        candidate = replace(before, state=queue.FAILED_SAFE, attempt_count=1)
        self.assertFalse(queue.validate_failed_safe_transition(before, candidate, expected_job_id="job-a"))

    def test_validator_rejects_wrong_target(self):
        before = self.running()
        self.assertFalse(queue.validate_failed_safe_transition(before, before, expected_job_id="job-a"))
    ...
    def test_validation_failure_no_clock(self):
        with mock.patch.object(queue, "validate_failed_safe_transition", return_value=False), mock.patch.object(queue, "datetime", wraps=datetime) as clock:
            updated, result = self.complete()
            clock.now.assert_not_called()
        self.assertIsNone(updated)
        self.assertEqual(result.transition_status, "REJECTED")
    ...
    def test_retry_decision_does_not_transition(self):
        value = self.running()
        before = asdict(value)
        with mock.patch.object(queue, "fail_job_safe", side_effect=AssertionError()) as fail:
            result = queue.assess_retry(value, "POLICY_VIOLATION")
        self.assertEqual(result.state, queue.FAILED_SAFE)
        self.assertEqual(asdict(value), before)
        fail.assert_not_called()

    def test_resume_decision_does_not_transition(self):
        value = job(state=queue.CHECKPOINTED)
        before = asdict(value)
        with mock.patch.object(queue, "fail_job_safe", side_effect=AssertionError()) as fail:
            result = queue.resume_from_checkpoint(value, checkpoint(value),
                now="2026-08-30T00:00:00Z", dependency_states={},
                environment_preflight_passed=True)
        self.assertEqual(result.status, queue.FAILED_SAFE)
        self.assertEqual(asdict(value), before)
        fail.assert_not_called()

    def test_no_automatic_side_effect_calls(self):
        value = self.running()
        with mock.patch.object(queue, "assess_retry", side_effect=AssertionError()) as retry, mock.patch.object(
                queue, "apply_approval", side_effect=AssertionError()) as approval, mock.patch.object(
                queue, "create_checkpoint", side_effect=AssertionError()) as cp, mock.patch.object(
                queue, "select_next_job", side_effect=AssertionError()) as select:
            self.assertEqual(self.complete(value)[1].transition_status, "APPLIED")
        for call in (retry, approval, cp, select):
            call.assert_not_called()

    def test_reason_codes_fixed(self):
        self.assertEqual(self.complete()[1].reason_code, "FAILED_SAFE_CONFIRMED")
        self.assertEqual(self.complete(job())[1].reason_code, "FAILED_SAFE_TRANSITION_INVALID")
        for key in ("reason", "payload", "exception"):
            with self.assertRaises(TypeError):
                queue.fail_job_safe(self.running(), expected_job_id="job-a", **{key: "fixture-secret"})
...
for _state in (queue.READY, queue.WAITING_APPROVAL, queue.FAILED_SAFE, queue.BLOCKED,
    ...
    setattr(FailedSafeContractTests, "test_reject_" + _state.lower(), reject_state(_state))
